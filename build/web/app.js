      ...
      this.filtred = function () {
        ...
        filtred = results.Results.filter(function (element) {
          if (filter_any) {
            ...
            var check = function check(search) {
              var rex = new RegExp(search);
              if (rex.test(title)) passed = true;else nopass = true;
            };

            if (qua) {
              if (qua == 1) check('4k|2160p|ultrahd');else if (qua == 2) check('fullhd|1080p');else check('720p');
            }

            if (hdr) {
              if (hdr == 1) check('hdr');else if (/hdr/.test(title)) nopass = true;
            }
